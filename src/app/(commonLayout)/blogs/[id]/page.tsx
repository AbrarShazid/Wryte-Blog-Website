...
// [ {id:asjkhdsa},{id:kjasdjasd},{id:digasdjhasdik} ]

export async function generateStaticParams() {
  const { data } = await blogService.getBlogPost();

  return data?.data?.data?.map((blog: BlogPost) => ({ id: blog.id })).splice(0,3);//splice for first 3 
}
...
export default async function IndividualPost({
  ...
  // const {id}=useParams()
  // useParams use korle client component hoye jabe
  ...
  const { data: blog } = data; //destructring and renaming
          ...
          <div className="flex flex-wrap gap-2">
            {blog.tags.map((tag: string) => (
              <Badge
                key={tag}
                variant="secondary"
                className="px-3 py-1 text-sm font-normal rounded-full underline text-blue-500"
              >
                #{tag}
              </Badge>
            ))}
